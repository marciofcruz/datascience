...
#### Interpretação inicial sobre o modelo
...
### j) Aplicar IV nas variáveis
```{r}
empbanc2 <- empbanc[, c("idade", "experiencia", "tempo_endereco", "renda", "debito_renda", "cred_deb", "outros_debitos", "classif")]

iv <- create_infotables(data = empbanc2, y = "classif")
kable(iv$Summary, caption = "IV")


modelo.c <- glm(classif ~
                experiencia +
                tempo_endereco +
                cred_deb +
                debito_renda, 
              family = binomial(link = 'logit'), data = empbanc2)

#Probabilidade estimada
empbanc2$probabilidade = predict(modelo.c, empbanc2, type = "response")
...
### j) VIF
```{r echo = F}
modelo.vif <- vif(modelo.c)
kable(modelo.vif)
```
Não foi encontrado no modelo apresentado variáveis com fator de inflação de variância.

### k) Ponto de Corte com maximização da Acurácia
```{r}
ponto <- cutpointr(empbanc2, probabilidade, classif, method = maximize_metric, metric = accuracy)
# summary(ponto) 
```
O ponto de corte otimizado é `r format(round(ponto$optimal_cutpoint, 4), nsmall = 4)`

### l) Ponto de Corte com minimização da diferença entre especificidade e sensibilidade
```{r}
ponto <- cutpointr(empbanc2, probabilidade, classif,
                   method = minimize_metric, metric = abs_d_sens_spec)
#summary(ponto) 
```
O ponto de corte otimizado é `r format(round(ponto$optimal_cutpoint, 4), nsmall = 4)`

### m) KS (Kolmogorov-Smirnov estatística)
```{r}
# KS_Stat(y_pred = empbanc2$probabilidade, y_true = empbanc2$classif)
ks_stat(actuals = empbanc2$classif, predictedScores = empbanc2$probabilidade)
```
### n) ROC
```{r}
plotROC(actuals=empbanc2$classif, predictedScores=empbanc2$probabilidade)
```



### o) Aplicando o corte para minimizar a diferença entre especificidade e sensibilidade
```{r}
empbanc2$predito <- as.factor(ifelse(empbanc2$probabilidade > 0.4902, 1, 0))

mc.tabela <- table(empbanc2$classif, empbanc2$predito)
kable(mc.tabela, caption="Resposta Observada x Resposta Predita")
...
##### Apresenta bom ajuste, já que há concentração dos casos na diagonal principal, confirmado pela acurácia de: `r format(round(mc.acuracia*100, 2), nsmall = 2)`%
##### Sendo que o objetivo do modelo é descobrir quem seja um mau pagador, usaremos a referência do indicador de sensibilidade: `r format(round(mc.sensibilidade*100, 2), nsmall = 2)`%.475 maus pagadores foram classificados corretamente e, 237 bons pagadores foram classificados incorretamente.
##### O resultado do índice de sensibilidade  demonstra um ajuste regular do modelo, já que é menor que 0.5.


### p) Interpretação Final
Anteriormente havíamos conseguido um índice de 40,97%. Porém, agora com o apoio do IV e da minimização da diferença entre sensibilidade e especificidade, conseguimos um valor de 42,87% de capacidade de um modelo acertar se um determinado cliente vai ser mau pagador.
